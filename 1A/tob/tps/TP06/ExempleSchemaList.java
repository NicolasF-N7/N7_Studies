...
public class ExempleSchemaList {
	...
	public static void main(String[] args)
	{
		...
		ArrayList<FigureGeometrique> schema = new ArrayList<FigureGeometrique>();	// le schéma
		...
		schema.forEach(x -> {x.afficher(); System.out.println();});
		...
		Consumer<FigureGeometrique> lambdaExpression = x -> x.dessiner(ecran);
		schema.forEach(lambdaExpression);

		// Translater le schéma
		System.out.println("Translater le schéma de (4, -3) : ");
		schema.forEach(x -> x.translater(4, -3));
		...
		// Dessiner le schéma sur l'écran graphique
		schema.forEach(x -> x.dessiner(ecran));
