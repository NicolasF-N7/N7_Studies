...
procedure Evaluer_Alea is
    ...
    CAPACITE: constant Integer := 6;   -- la capacité du tableau
    ...
    is
        ...
        Tab_Tirages: Tableau_Entier;--Tableau de longueur Borne
        Alea: Integer;
        Cp_Min: Integer;
        Cp_Max: Integer;
    begin
        ...
        for i in 1..Taille loop
            ...
            Tab_Tirages(Alea) := Tab_Tirages(Alea) +1;
        end loop;

        Cp_Min :=Tab_Tirages(1);
        Cp_Max :=Tab_Tirages(1);

        for i in 1..Borne loop
            if Tab_Tirages(i) > Cp_Max then
                Cp_Max := Tab_Tirages(i);
            elsif Tab_Tirages(i) < Cp_Min then
                Cp_Min := Tab_Tirages(i);
            end if;
        end loop;
