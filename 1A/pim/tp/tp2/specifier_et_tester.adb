...
procedure Specifier_Et_Tester is
    ...
    -- Efficaciwith Ada.Text_IO;
use Ada.Text_IO;
té : faible car on utilise une version naïve de l'algorithme
    -- d'Euclide.
    ...
    ---Calcul le nombre de jour du mowith Ada.Text_IO;
use Ada.Text_IO;
is spécifié en entrée
    ---Param:
    --- M : in entier représente le mois
    ---Retour:
    --- J entier -- le nb de jour dans le mois M
    --- necessite 1<=M<=12
    procedure calc_nb_jour_mois (M: in Integer) return Integer with
            Pre => 1<= M <= 12
            Post => 1 <= J <= 31
    is
    begin
