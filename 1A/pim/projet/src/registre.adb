...
package body Registre is
    ...
    begin
        if Donnee.Id = Capacite then
            return Present_Liste(Registre.Tableau(Capacite), Donnee);--Attention indice tableau dans 1..N
        else
    ...
    begin
        if Id = Capacite then
            return Present_Liste(Registre.Tableau(Capacite), Obtenir_Donnee(Registre, Id));
        else
    ...
    begin
        ...
        if Donnee.Id = Capacite then
            Ajouter_Liste(Registre.Tableau(Capacite), Donnee);
        else
    ...
    begin
        if Id = Capacite then
            Curseur := Registre.Tableau(Capacite);
        else
    ...
    begin
        ...
        if Old_Donnee.Id = Capacite then
            Liste := Registre.Tableau(Capacite);
        else
    ...
    begin
        if Donnee.Id = Capacite then
            ...
            i := Donnee.Id mod Capacite;
        end if;

        if Liste_Donnee.Present_Liste(Registre.Tableau(i), Donnee) then
            Supprimer_Liste(Registre.Tableau(i), Donnee);
            Registre.Nb_Individus := Registre.Nb_Individus -1;
        else
            null;
        end if;
    ...
    begin--ATTENTION ne pas faire  Liste := Registre.Tableau(Id mod Capacite); puis supprimer Liste, Donnee associee a Id: ne marche pas dans le cas ou on supprime la tete de liste
        ...
        if Liste_Donnee.Present_Liste(Registre.Tableau(i), Obtenir_Donnee(Registre, Id)) then
            Supprimer_Liste(Registre.Tableau(i), Obtenir_Donnee(Registre, Id));
            Registre.Nb_Individus := Registre.Nb_Individus -1;
        else
            null;
        end if;
