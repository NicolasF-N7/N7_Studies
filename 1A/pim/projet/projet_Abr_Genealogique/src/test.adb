...
procedure Test is
   ...
   -- pragma Unreferenced (Afficher_Liste);

    procedure Aff_Elmt_Donnee(x: in T_Donnee) is
    begin
        Put_Line("Id: " & Integer'Image(x.Id) & ", Prenom: " & To_String(x.Prenom) & ", Nom: " & To_String(x.Nom));
    end Aff_Elmt_Donnee;
...
begin
    ...
    --Put_Line("Prenom P1 apres modif: " & To_String(Obtenir_Donnee(Reg,1).Prenom));
    ...
    New_Line;
    New_Line;
    Put_Line("Taille Reg: " & Integer'Image(Taille_Registre(Reg)));
    Put_Line("---Supprimer P1---");
    Supprimer_Registre(Reg, 1);
    Supprimer_Registre(Reg, 2);
    Supprimer_Registre(Reg, 3);
    Put_Line("Taille Reg: " & Integer'Image(Taille_Registre(Reg)));

    Detruire_Registre(Reg);
    Liste_Donnee.Detruire_Liste(Liste_H);
