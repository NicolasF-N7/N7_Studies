...
% Affichage du canal R
    subplot(2,2,2) % 1ère ligne, 2ème colonne
    imagesc(reshape(C(:,1), size(I(:,:,1))))
    axis off image
    title('Canal R','FontSize',20)
    
% Affichage du canal V
    subplot(2,2,3) % 2ème ligne, 1ère colonne
    imagesc(reshape(C(:,2), size(I(:,:,1))))
    axis off image
    title('Canal V','FontSize',20)
    
% Affichage du canal B
    subplot(2,2,4) % 2ème ligne, 2ème colonne
    imagesc(reshape(C(:,3), size(I(:,:,1))))
    axis off image
    title('Canal B','FontSize',20)

%% Affichage de l'image dans le nouveau repère

figure('Name','Nuage de pixels dans le repere ACP',...
        'Position',[0.61*L,0.1*H,0.38*L,0.6*H])
     ...
     ylabel('C2','FontWeight','bold')
     zlabel('C3','FontWeight','bold')
     title({'Représentation 3D des pixels' ...
            'dans l''espace ACP'},'FontSize',20)
     rotate3d 