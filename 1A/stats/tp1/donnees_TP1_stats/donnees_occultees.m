...
if theta1 < theta2
    ind_donnees_bruitees_occultees = (theta_donnees_bruitees >= theta1 & theta_donnees_bruitees <= theta2);
else
    ind_donnees_bruitees_occultees=(0 <= theta_donnees_bruitees & theta_donnees_bruitees <= theta2) | (theta1 <= theta_donnees_bruitees & theta_donnees_bruitees <= 2*pi);
end

x_donnees_bruitees_occultees = x_donnees_bruitees(ind_donnees_bruitees_occultees);
y_donnees_bruitees_occultees = y_donnees_bruitees(ind_donnees_bruitees_occultees);

% Affichage des donnees bruitees occultees:
plot(x_donnees_bruitees_occultees,y_donnees_bruitees_occultees,'k+','MarkerSize',10,'LineWidth',2);
axis(bornes);
