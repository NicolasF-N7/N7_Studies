...
subroutine facto(m, ldm, n, ipivot, p, q, ierr)
  ...
  !write(6,*)"Factorisation non implantee" 
    ...
    do k = 1,(n-1)
        if (m(k,k).le.eps) then
            ierr = ierr + 1
        endif
        m(k+1:n, k) = m(k+1:n,k) / m(k,k)
        !allocate (tmp(n-k, n-k))
        !tmp = matmul(RESHAPE(m(i+1:n, i),(/n-i-1,1/)), RESHAPE(m(i, i+1:n),(/n-i-1,1/)))
        !tmp = matmul(m(i+1:n, i), m(i, i+1:n))
        m(k+1:n, k+1:n) = m(k+1:n, k+1:n) - m(k+1:n, k) * m(k, k+1:n)
        !do i=k+1,n
        !    do j = k+1,n
