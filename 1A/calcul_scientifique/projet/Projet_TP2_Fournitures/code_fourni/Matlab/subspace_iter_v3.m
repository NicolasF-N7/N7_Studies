...
function [ W, V, n_ev, it, flag ] = subspace_iter_v1(A, m, percentage, p, eps, maxit)
    ...
    while (~conv & k < maxit),
        ...
        if (nb_c > 0) % pour éviter d'avoir 1:0
            Y(:, 1:nb_c) = Vr(:, 1:nb_c);
        end
        Y(:, (nb_c+1):end) = A * Vr(:, (nb_c+1):end);
        for i=2:p
            Y(:, (nb_c+1):end) = A * Y(:, (nb_c+1):end);
        end
        % Ici on a Y = A^p * Vr
        %% orthogonalisation
        Vr = mgs_block(Y, p);
        ...
        [Wr, Vr] = rayleigh_ritz_projection(A, Vr(:, nb_c+1:end));
