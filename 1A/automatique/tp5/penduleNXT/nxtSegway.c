...
float prod_scal(float x[4], float u[4]){
  int sum=0;
  for(int i=0; i<4; i++){
    sum +=x[i]*u[i];
  }
...
TASK(pendule) {
  ...
  case(CONTROL_MODE):
    ...
    theta_m=getMotorAngle();
    psi_point=getGyro(gyro_offset);
    dt = delta_t();
    psi_avant = x[1];
    psi = psi_point*dt + psi_avant;
